...
<body>
    <form action="{{ route('admin.submit') }}" method="GET">
    ...
    <form action="{{ route('departmentdata.submit') }}" method="post">
        @csrf
        <aside id="sidebar-multi-level-sidebar" class="fixed top-15 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
            <div class="flex flex-col w-56 min-h-lvh bg-blue-300">
                <button class="mb-2 mt-9 text-1xl text-black font-semibold" id="fetchdepartment" onclick="fetchDepartmentData()">Department</button>
        ...
        <div id="dept-table" class="hidden  rounded-lg overflow-hidden px-3 mt-4 ">
     
            <table class="table table-dark table-bordered table-center  w-52 ml-52 mr-10 " >
     ...
     <form action="{{route('coursedata.submit')}}" method="POST">
        ...
        <div id="course-table" class="hidden  rounded-lg overflow-hidden px-3 mt-4 ">
        ...
        <form action="{{ route('userdata.submit') }}" method="post">
            ...
            <div id="user-table" class="hidden rounded-lg overflow-hidden px-1 mt-4">
                <table class="table table-dark table-bordered table-center ml-auto mr-auto  max-w-md ">
    ...
    $(document).ready(function(){
        $('#fetchuser').click(function(e){
            ...
            $('#user-table-body').empty();//after displaying once so these data will be clear so that when we click again those row will not repaeted
            $.ajax({
                type:"POST",
                ...
                success:function(response){
                    ...
                    $.each(response.$users, function(index, value){
                        ...
                        var row = '<tr>' +
                            '<td class="px-4 py-2">' + value.userId + '</td>' +
                            '<td class="px-4 py-2">' + value.name + '</td>' +
                            '<td class="px-4 py-2">' + value.email + '</td>' +
                            '<td class="px-4 py-2">' + value.password + '</td>' +
                            '<td class="px-4 py-2">' + value.type + '</td>' +
                            '<td class="px-4 py-2"><button type="button" value="" class="edit_dept bg-blue-500 hover:bg-blue-700 text-white px-4 py-2 rounded-lg focus:outline-none">Edit</button></td>' +
                            '<td class="px-4 py-2"><button type="button" class="delete_dept bg-red-500 hover:bg-red-700 text-white px-4 py-2 rounded-lg focus:outline-none">Delete</button></td>' +
                            '</tr>';
